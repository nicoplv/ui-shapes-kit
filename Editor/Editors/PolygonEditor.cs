...
using Polygon = NicoplvTools.UiShapesKit.Shapes.Polygon;

[CustomEditor(typeof(Polygon))]
public class PolygonEditor : GraphicEditor
{
	Polygon polygon;
	NicoplvTools.UiShapesKit.ShapeUtils.PointsList.PointListsProperties pointListsProperties;
	...
	void OnSceneGUI()
	{
		...
		for (int i = 0; i < pointListsProperties.PointListProperties.Length; i++)
		{
			if (
				...
				pointListsProperties.PointListProperties[i].GeneratorData.Generator == NicoplvTools.UiShapesKit.ShapeUtils.PointsList.PointListGeneratorData.Generators.Custom
			) {
